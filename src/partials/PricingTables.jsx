...
function PricingTables({ planInfos }) {
  ...
  const handleClick = async (plan) => {
    ...
    const result = await axios.post(
      "https://seashell-app-2-n2die.ondigitalocean.app/api/v1/checkout/sk_test_51MbAZNKrjQpaXbt11BqblQtCpraeA1nV1nmEX9rIaZdBpJQlIwrjK2aijRGVmo8WH7H5unbbUL7jRjrRbVagoswv00FlFbimKp",
      {
        name: user?.fullName,
        phone: "123456789",
        email: user?.email,
  ...
  const getHasSubscription = async () => {
    await axios
      .get(
        `https://seashell-app-2-n2die.ondigitalocean.app/api/v1/has-subscription/${user?.customerId}`
      )
      .then((res) => {
  ...
  const getSubscriptionStatus = async () => {
    await axios
      .get(
        `https://seashell-app-2-n2die.ondigitalocean.app/api/v1/subscriptions/customer/${user?.customerId}`
      )
      .then((res) => {
        axios
          .get(
            `https://seashell-app-2-n2die.ondigitalocean.app/api/v1/${res.data?.subscriptionIds[0]}/status`
          )
          .then((response) => {
  ...
  const cancelSubscription = async () => {
    await axios
      ...
      .then((res) => {
        axios
          .post(
            `https://seashell-app-2-n2die.ondigitalocean.app/api/v1/cancel-subscription`,
            {
            ...
            <div className="max-w-xs mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-4 items-start sm:max-w-none md:max-w-2xl lg:max-w-none">
              ...
              >
                ...
                <div onClick={() => handleClick("basic")} className="mt-24">
                  <a
                    className="btn-sm text-white bg-[#3b82f6] hover:bg-[#145aca] w-full"
                    href="#0"
                  >
                    Go Premium
                  </a>
