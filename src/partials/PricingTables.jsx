...
function PricingTables({ planInfos }) {
  ...
  const handleClick = async (plan) => {
    ...
    const result = await axios.post(
      "https://seashell-app-8amlb.ondigitalocean.app/api/v1/checkout/sk_test_51MbAZNKrjQpaXbt11BqblQtCpraeA1nV1nmEX9rIaZdBpJQlIwrjK2aijRGVmo8WH7H5unbbUL7jRjrRbVagoswv00FlFbimKp",
      {
        name: user?.fullName,
        phone: "123456789",
        email: user?.email,
  ...
  const getHasSubscription = async () => {
    await axios
      .get(
        `https://seashell-app-8amlb.ondigitalocean.app/api/v1/has-subscription/${user?.customerId}`
      )
      .then((res) => {
  ...
  const getSubscriptionStatus = async () => {
    await axios
      .get(
        `https://seashell-app-8amlb.ondigitalocean.app/api/v1/subscriptions/customer/${user?.customerId}`
      )
      .then((res) => {
        axios
          .get(
            `https://seashell-app-8amlb.ondigitalocean.app/api/v1/${res.data?.subscriptionIds[0]}/status`
          )
          .then((response) => {
  ...
  const cancelSubscription = async () => {
    await axios
      ...
      .then((res) => {
        axios
          .post(
            `https://seashell-app-8amlb.ondigitalocean.app/api/v1/cancel-subscription`,
            {
        ...
        <div className="py-12 md:py-20">
          ...
          <div className="max-w-3xl mx-auto text-center pb-12">
            <h2 className="h3 font-red-hat-display mb-4 text-gray-100">
              {hasSubscription
                ? `You're now on ${user?.Plan} plan. Your subscription is ${subscriptionStatus}`
                : "Start building for free, then upgrade to a plan to unleash your content."}
            </h2>
              ...
              <div className="flex flex-col justify-center">
                <div className="flex md:flex-row flex-col md:space-x-3">
                  <div
                    ...
                    className="mb-2 rounded-md  font-bold text-center z-6 text-white bg-gradient-to-r from-[#6366ff] to-[#373afd] py-3 w-full md:w-1/2"
                  >
                    Download Invoice
                  </div>
                  <div
                    onClick={cancelSubscription}
                    className="mb-2 rounded-md  cursor-pointer  font-bold text-center z-6 text-white bg-gradient-to-r from-[#f22323] to-[#e61911] py-3  w-full md:w-1/2"
                  >
                    {cancelMessage !== null
                      ? cancelMessage
                      : " Cancel Subscription"}
          ...
          {!hasSubscription && !isPlansLoading && (
            <div className="max-w-xs mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-4 items-start sm:max-w-none md:max-w-2xl lg:max-w-none">
              {/* Pricing table 1 */}
              <div
                className="flex flex-col h-full p-6 bg-gray-800 shadow border-2 border-[#3b82f6]"
                data-aos="fade-down"
              >
                <div className="grow mb-4 pb-4">
                  ...
                  <div className="font-red-hat-display inline-flex items-baseline mb-b mt-6">
                    ...
                    <span className="h3 text-white">
                      {(planInfos[2]?.amount / 100).toFixed(2)}
                    </span>
                ...
                >
                  <div className="grow mb-4 pb-4">
                    <div className="flex justify-between items-center mb-1">
                      <div className="text-xl font-bold font-red-hat-display"></div>
                      <div className="inline-flex px-3 py-1 text-xs font-medium text-white  bg-[#3b82f6]  rounded-full">
                        -5%
                      </div>
                    </div>
                  ...
                  <div className="font-red-hat-display inline-flex items-baseline mb-2">
                    <span className="h4 text-gray-400">$</span>
                    <span className="h3 text-white">
                      {(planInfos[1]?.amount / 100).toFixed(2)}
                    </span>
                    <span className="font-medium text-gray-500 "> / month</span>
                  </div>
                  {isLoading && (
                ...
                >
                  ...
                  <div className="font-red-hat-display inline-flex items-baseline mb-2">
                    <span className="h4 text-gray-400">$</span>
                    <span className="h3 text-white">
                      {(planInfos[0]?.amount / 100).toFixed(2)}
                    </span>
                    <span className="font-medium text-gray-500 "> / month</span>
                  </div>
                </div>
